...
export class SidebarNavComponent implements OnDestroy {
  ...
  toggleMenu(): void { this.menuOpen.update((v) => !v); this.syncBodyScroll(); }
  closeMenu(): void { this.menuOpen.set(false); this.syncBodyScroll(); }
  onNavClick(): void { this.closeMenu(); }

  @HostListener('window:resize')
  onResize(): void {
    if (!isPlatformBrowser(this.platformId)) return;
    if (!window.matchMedia('(max-width: 1023px)').matches) this.menuOpen.set(false);
    this.syncBodyScroll();
  }
  ...
  private syncBodyScroll(): void {
    ...
    this.doc.body.style.overflow = mobile && this.menuOpen() ? 'hidden' : '';
  }
}
